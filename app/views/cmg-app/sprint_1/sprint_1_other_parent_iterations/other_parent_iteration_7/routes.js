...
router.post('/about_other_parent/other_parent_date_of_birth', function (req, res) {
  let ninoQuery = req.session.data['nino_query']
  if (ninoQuery === 'yes') {
    res.redirect('other_parent_national_insurance_number')
  } else {
    res.redirect('other_parent_date_of_birth_page_2')
  }
});

//Normal route with continue - parents date of birth page 2
router.post('/about_other_parent/other_parent_date_of_birth_page_2', function (req, res) {
  res.redirect('other_parent_national_insurance_number')
});
