...
router.post('/about_applicant/applicants_contact_files/applicants_contact_details', function (req, res) {
  res.redirect('/cmg-app/sprint_1/sprint_1_main_prototype/reviews/review_your_answers_applicants_details')
});
...
let child216To20 = req.session.data['child-2-in-full-time-education']
let child2married = req.session.data['child-2-married']

if (child2DateOfBirthYear < 2000 || child216To20 === 'No' || child2married === 'Yes'){
...
router.post('/service_choice/collect_and_pay_bank_details', function (req, res) {
  ...
  router.post('/personal_security/personal_security_details', function (req, res) {
    ...
    else if (serviceChoice === 'Collect and Pay' && addAnotherChild === 'Yes') {
      res.redirect('../reviews/review_your_answers_collect_and_pay_plus_child')
    }
...
router.post('/about_qualifying_child_two/child_name', function (req, res) {
  ...
  if (addAnotherName === 'yes') {
    res.redirect('../about_qualifying_child/child_date_of_birth')
  } else {
    res.redirect('child_date_of_birth')
  }
});
...
if (child2married === 'No') {
  res.redirect('shared_care')
} else if (child2married === 'Yes' && addAnotherName === 'yes') {
  res.redirect('../dropouts/dropout_child_reasons_2')
} else {
  res.redirect('../dropouts/dropout_child_reasons')
}
