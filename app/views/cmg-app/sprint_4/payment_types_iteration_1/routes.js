...
router.post('/about_applicant/applicants_address', function (req, res) {
  res.redirect('applicants_contact_files/applicants_contact_details')
});
...
//Normal route with continue - Applicants address manual 
router.post('/about_applicant/applicants_contact_files/applicants_contact_address_manual_input', function (req, res) {
...
router.post('/about_applicant/applicants_contact_files/applicants_contact_details', function (req, res) {
  res.redirect('/cmg-app/sprint_2/sprint_2_main_prototype/reviews/review_your_answers_applicants_details')
});
...
router.post('/reviews/review_your_answers_applicants_details', function (req, res) {
  res.redirect('../tasklists/tasklist_2')
});
...
router.post('/about_qualifying_child/child_in_full_time_education', function (req, res) {
  ...
  } else if (child16To20 === 'No' && addAnotherName === 'yes') {
    res.redirect('../dropouts/dropout_child_reasons_2')
  } else {
    res.redirect('../dropouts/dropout_child_reasons')
  }
...
router.post('/about_qualifying_child/married_civil_partnership', function (req, res) {
  ...
  if (child1married === 'No') {
    res.redirect('consent_order')
  } else if (child1married === 'Yes' && addAnotherName === 'yes') {
    res.redirect('../dropouts/dropout_child_reasons_2')
  } else {
    res.redirect('../dropouts/dropout_child_reasons')
  }
...
let child2DateOfBirthYear = req.session.data['child-2-date-of-birth-year']
  ...
  if (addAnotherName === 'yes') {
    ...
    if (child2DateOfBirthYear < 2004 && child2DateOfBirthYear > 1999 ) {
      ...
      res.redirect('../about_qualifying_child_two/consent_order')
    }
...
router.post('/reviews/review_your_answers_other_parent', function (req, res) {
  res.redirect('../tasklists/tasklist_4')
});
...
router.post('/service_choice/choose_service', function (req, res) {
  ...
  if (serviceChoice === 'Direct Pay') {
    res.redirect('../tasklists/tasklist_5')
  }
...
router.post('/service_choice/collect_and_pay_bank_details', function (req, res) {
  ...
  router.post('/personal_security/personal_security_details', function (req, res) {
    res.redirect('../tasklists/tasklist_6')
  });
...
router.post('/about_qualifying_child_two/child_name', function (req, res) {
  ...
  let child1DateOfBirthYear = req.session.data['child-1-date-of-birth-year']
    ...
    if (child1DateOfBirthYear < 2004 && child1DateOfBirthYear > 1999 ) {
      res.redirect('../about_qualifying_child/child_in_full_time_education')
    } else if (child1DateOfBirthYear < 2000 && addAnotherName === 'yes' ){
      res.redirect('../dropouts/dropout_child_reasons_2')
    } else if (child1DateOfBirthYear < 2000 && addAnotherName === 'no'){
      res.redirect('../dropouts/dropout_child_reasons')
    } else {
      res.redirect('../about_qualifying_child/consent_order')
    }
...
router.post('/about_qualifying_child_two/child_check_answers', function (req, res) {
    ...
    if (child1DateOfBirthYear < 2000 || child16To20 === 'No' || child1married === 'Yes'){
      res.redirect('add_another_child_second_child_only') 
      }
      else {
...
router.post('/exemption_pages/exemption_applicant_northern_ireland', function (req, res) {
  res.redirect('../about_applicant/applicants_contact_files/applicants_contact_details')
});
...
router.post('/service_choice/contact_with_other_parent', function (req, res) {
  ...
  if (contactOtherParent === 'yes'){
    res.redirect('doubts_about_payment') 
    }
    else {
      res.redirect('payments_to_be_made_1') 
    ...
    router.post('/service_choice/doubts_about_payment', function (req, res) {

      let paymentDoubts = req.session.data['payment-doubts']
    
      if (paymentDoubts === 'yes'){
        res.redirect('payments_to_be_made_2') 
        }
        else {
    ...
    router.post('/service_choice/collect_and_pay', function (req, res) {
      res.redirect('results_page')
    });
  

            //Normal route with continue - exemption domestic abuse
            router.post('/service_choice/payments_to_be_made_1', function (req, res) {
              res.redirect('results_page')
            });
          

            //Normal route with continue - exemption domestic abuse
            router.post('/service_choice/payments_to_be_made_2', function (req, res) {
              res.redirect('results_page')
            });
