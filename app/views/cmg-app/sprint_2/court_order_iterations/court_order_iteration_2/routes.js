...
router.post('/about_qualifying_child/court_order_details', function (req, res) {

  let courtOrderYrs = req.session.data['child-1-court-order-year']
  let courtOrder = req.session.data['child-1-court-order']

   if (courtOrderYrs > 2017 || courtOrderYrs < 2004) {
    res.redirect('../dropouts/dropout_court_order')
  }  else {
    res.redirect('court_order_type')
  }
});

//Normal route with continue - Review Answers
router.post('/about_qualifying_child/court_order_type', function (req, res) {
  res.redirect('court_order_address')
});
...
router.post('/about_qualifying_child/court_order_under_12_months', function (req, res) {
  ...
  let maintenanceOrderRequest = req.session.data['maintenance-order-request']

  if (maintenanceOrderRequest === 'no') {
    res.redirect('../tasklists/tasklist_1')
  }  else {
    res.redirect('court_order_details')
  }
});
